  ...
  sampleEnvironment(lanes, isNightManual) {
    ...
    return {
      ...
      processed: {
        ...
        dryWasteFill: Math.min(100, Math.max(0, dustbinFillPercent)),
        dryWasteRemaining: Math.max(0, 100 - dustbinFillPercent),
        wetWasteFill: Math.min(100, Math.max(0, Math.round(dustbinFillPercent * 0.6))),
        wetWasteRemaining: Math.max(0, 100 - Math.round(dustbinFillPercent * 0.6)),
        slot1Occupied,
        slot2Occupied,
